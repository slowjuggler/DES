...
int rnd_time[16] = {1, 1, 2, 2, 2, 2, 2, 2,
                    1, 2, 2, 2, 2, 2, 2, 1};  // round counter
...
char tabS[8]
         [64] = {{14, 4,  13, 1, 2,  15, 11, 8,  3,  10, 6,  12,
                  5,  9,  0,  7,  // 64 bit S tables (8 tables for 64 bit)
                  0,  15, 7,  4, 14, 2,  13, 1,  10, 6,  12, 11,
                  9,  5,  3,  8, 4,  1,  14, 8,  13, 6,  2,  11,
                  15, 12, 9,  7, 3,  10, 5,  0,  15, 12, 8,  2,
...
void rfunc(char *left, char *right, char *sk) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    columns = ((output_2[i * 6 + 1] - '0') << 3) +
              ((output_2[i * 6 + 2] - '0') << 2) +
              ((output_2[i * 6 + 3] - '0') << 1) + (output_2[i * 6 + 4] - '0');
    for (j = 3; j >= 0; j--) {
      *(output_3 + (i * 4 + 3 - j)) =
          ((tabS[i][rows * 16 + columns] >> j) & 1) + '0';
    }
  }
  *(output_3 + 32) = '\0';
